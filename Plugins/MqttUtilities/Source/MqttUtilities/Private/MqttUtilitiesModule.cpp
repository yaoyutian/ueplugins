...
void FMqttUtilitiesModule::StartupModule()
{
	...
	dynaSSLLib = FPlatformProcess::GetDllHandle(*(DLLPath + "libssl-3-x64.dll"));
	dynaCryptoLib = FPlatformProcess::GetDllHandle(*(DLLPath + "libcrypto-3-x64.dll"));
...
void FMqttUtilitiesModule::ShutdownModule()
{
	...
	if (dynaSSLLib)
	{
		FPlatformProcess::FreeDllHandle(dynaSSLLib);
		dynaSSLLib = nullptr;
	}

	if (dynaCryptoLib)
	{
		FPlatformProcess::FreeDllHandle(dynaCryptoLib);
		dynaCryptoLib = nullptr;
	}
#endif
}
